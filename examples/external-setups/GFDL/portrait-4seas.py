...
out1_rel, out2_rel, out3_rel, out4_rel = [
    np.ma.masked_all(
        (len(vars), len(mods)), np.float32) for _ in range(4)]  # Define arrays to fill
    ...
    for mn, mod in enumerate(mods):
        try:
            out1_rel[
                vn,
                mn] = float(
                var_cmip5_dics[var][mod]["defaultReference"]["r1i1p1"]["global"]['rms_xy_djf_GLB'])
            out2_rel[
                vn,
                mn] = float(
                var_cmip5_dics[var][mod]["defaultReference"]["r1i1p1"]["global"]['rms_xy_mam_GLB'])
            out3_rel[
                vn,
                mn] = float(
                var_cmip5_dics[var][mod]["defaultReference"]["r1i1p1"]["global"]['rms_xy_jja_GLB'])
            out4_rel[
                vn,
                mn] = float(
                var_cmip5_dics[var][mod]["defaultReference"]["r1i1p1"]["global"]['rms_xy_son_GLB'])
