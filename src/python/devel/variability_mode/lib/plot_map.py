def plot_map(mode, model, syear, eyear, season, eof1, frac1, output_file_name):
  ...
  if mode == 'NAM' or mode == 'SAM':
    ...
    p.type = 'lambert'
  elif mode == 'PDO_teleconnection' or mode == 'PDO_CBF_teleconnection':
    p.type = 'robinson' 
  else:
    p.type = int('-3')
  iso.projection = p
  xtra = {}
  if mode == 'PDO_teleconnection' or mode == 'PDO_CBF_teleconnection':
    xtra = {}
  elif mode == 'SAM' or mode == 'SAM_teleconnection' or mode == 'SAM_CBF_teleconnection':
