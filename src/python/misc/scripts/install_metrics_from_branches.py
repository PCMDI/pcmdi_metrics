...
c.add_argument("-M","--no-mesa", default=False, action="store_true",help="turn on/off mesa")
c.add_argument("-n","--no-nightly", default=False, action="store_true",help="turn on/off mesa")
b = P.add_argument_group('Repositories')
b.add_argument("-v","--vcs",default=["master"],nargs="*",help="vcs branches to merge")
b.add_argument("-c","--cdms",default=["master"],nargs="*",help="cdms branches to merge")
b.add_argument("-m","--metrics",default=["master"],nargs="*",help="metrics branches to merge in")
l = P.add_argument_group('Local Setup')
l.add_argument("-g","--git",default=os.path.expanduser("~/git"),help="top directory where you will clone your git repos")
...
# Create conda env
execute_cmd("conda create -y -n %s %s -c uvcdat eztemplate vcsaddons%s cdms2 vcs%s hdf5=1.8.16 "% (args.env,nightly,pname,pname))
...
execute_cmd("git clone git://github.com/"% metrics_repo,path=args.git)
...
print("You should be good to go, we merged vcs branches: '%s' cdms branches: '%s' and metrics branches '%s' into conda env: '%s'" % (" ".join(args.vcs)," ".join(args.cdms)," ".join(args.metrics),args.env))
