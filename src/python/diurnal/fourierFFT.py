...
On output: c[i] = mean value at each gridpoint (i) in the time series (= constant term in Fourier series)  
        ...
        c = S[:, 0]
        tmax      = numpy.zeros((nGridPoints, 3)) # time of maximum for nth component (n=0 => diurnal, n=1 => semi...)
        maxvalue  = numpy.zeros((nGridPoints, 3)) # value of maximum for nth component (= 1/2 peak-to-peak amplitude)
        for n in range(3):
            maxvalue[:, n] = S[:, n+1] + S[:, -n-1] # Adding first + last terms, second + second-to-last, ...
...
On output: c[i,j] = mean value at each gridpoint (i,j) in the time series ("zeroth" term in Fourier series)  
        ...
        I = x.shape[1]
        J = x.shape[2]
        tmax      = numpy.zeros((3, I, J)) # time  of maximum for nth component (n=0 => diurnal, n=1 => semi...)
        maxvalue  = numpy.zeros((3, I, J)) # value of maximum for nth component (= 1/2 peak-to-peak amplitude)
