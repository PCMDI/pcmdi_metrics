...
# but such areas should count for little since the vector amplitudes there are small.


# This modifiction of ./savg_fourier.py has the PMP Parser "wrapped" around it, so it's executed with both input and
# output parameters specified in the Unix command line. For example:

# ---> python savg_fourierWrappedInOut.py --mns "['MIROC5', 'CCSM4']" -mo 07 -y1 1999 -y2 2005 --outnj savgDiurnalFourierJuly
...
def vectoravg(hr1, hr2, clocktype):
    ...
    sin_avg = (MV2.sin(hrs_to_rad(hr1, clocktype)) + MV2.sin(hrs_to_rad(hr2, clocktype))) / 2
...
def spacevavg(tvarb1,tvarb2,sftlf,model):
    ...
    for harmonic in harmonics:
        ...
        sin_avg_ocn /= (1 - glolf)
        # Amplitude and phase:
        amp_avg_glo = MV2.sqrt(sin_avg_glo**2 + cos_avg_glo**2) # * 86400 Convert kg/m2/s -> mm/d?
        amp_avg_lnd = MV2.sqrt(sin_avg_lnd**2 + cos_avg_lnd**2) # * 86400 Convert kg/m2/s -> mm/d?
        amp_avg_ocn = MV2.sqrt(sin_avg_ocn**2 + cos_avg_ocn**2) # * 86400 Convert kg/m2/s -> mm/d?
...
OUT = pcmdi_metrics.io.base.Base(os.path.abspath(args.output_directory),os.path.basename(jsonname))
disclaimer = open(
    os.path.join(
        sys.prefix,
        "share",
...
# Accumulate output from each model (or observed) data source in the Python dictionary.
