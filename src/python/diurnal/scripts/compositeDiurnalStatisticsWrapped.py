...
# This modifiction of ./compositeDiurnalStatistics.py will have the PMP Parser "wrapped" around it, so that it can be executed
...
# 'ACCESS1-0', 'ACCESS1-3', 'bcc-csm1-1', 'bcc-csm1-1-m', 'BNU-ESM',   'CCSM4',  'CMCC-CM',      'CNRM-CM5',     'EC-EARTH',
# 'FGOALS-g2', 'GFDL-CM3',  'GFDL-ESM2M', 'GISS-E2-R',    'GISS-E2-H', 'inmcm4', 'IPSL-CM5A-LR', 'IPSL-CM5A-MR',
# 'MIROC4h',   'MIROC5',    'MIROC-ESM',  'MIROC-ESM-CHEM'

import cdms2, genutil, MV2, os
import sys
import glob
import cdtime
    ...
    if dataname not in skipMe:
        ...
        for year in range(args.firstyear, args.lastyear+1):
            ...
            if year == args.firstyear: # The following tasks need to be done only once, extracting metadata from first-year file: 
                ...
                lons = modellons[:] # Longitude values are needed later to compute Local Solar Times.
                print '  Creating temporary storage and output fields ...'
                tvslice   =     MV2.zeros((N, dayspermo,        nlats, nlons)) # Sorts tvarb into separate GMTs for one year
                concatenation = MV2.zeros((N, dayspermo*nYears, nlats, nlons)) # Concatenates tvslice over all years
        ...
        # For each GMT, take mean and standard deviation over all years for the chosen month:
        ...
        for iGMT in range(N):
            ...
            avgvalues[iGMT] = MV2.average(concatenation[iGMT], axis = 0) # Assumes first dimension of input ("axis#0") is time
        ...
        avgoutfile = ('%s_%s_%s_%s-%s_diurnal_avg.nc') % (varbname, dataname, monthname, str(args.firstyear), str(args.lastyear))
        stdoutfile = ('%s_%s_%s_%s-%s_diurnal_std.nc') % (varbname, dataname, monthname, str(args.firstyear), str(args.lastyear))
        LSToutfile = ('%s_%s_LocalSolarTimes.nc' % (varbname, dataname))
        if not os.path.exists(args.output_directory):
            os.makedirs(args.output_directory)
        f = cdms2.open(os.path.join(args.output_directory,avgoutfile), 'w')
        g = cdms2.open(os.path.join(args.output_directory,stdoutfile), 'w')
        h = cdms2.open(os.path.join(args.output_directory,LSToutfile), 'w')
        f.write(avgvalues)
        g.write(stdvalues)
        h.write(LSTs)
        f.close()
