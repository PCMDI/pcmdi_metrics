...
`variability_mode.eof_classification` and `sea_ice.calc_iiee_annual_cycle` are available in `pcmdi_metrics (> v4.1)` or higher.
`mjo.compute_mjo_ewr_from_dataset` is available in `pcmdi_metrics (> v4.1.1)` or higher.
