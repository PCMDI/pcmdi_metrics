...
json_files = glob.glob(
    ...
    'metrics_results/cmip5clims_metrics_package-picontrol/cmip5_test/*.json')

json_files_acme = glob.glob(
    '/export/gleckler1/141126_metrics-acme/sampletest_141204/*.json')

for f in json_files_acme:
    json_files.append(f)
...
for bad in ["References", "RegionalMasking",
...
for var in vars:  # LOOP OVER VARIABLE
    ...
    for mod in mods:  # LOOP OVER MODEL
        try:
            rms1 = var_cmip5_dics[var][mod]["defaultReference"][
                "r1i1p1"]["global"]['rms_xy_djf_GLB']
            rms2 = var_cmip5_dics[var][mod]["defaultReference"][
                "r1i1p1"]["global"]['rms_xy_mam_GLB']
            rms3 = var_cmip5_dics[var][mod]["defaultReference"][
                "r1i1p1"]["global"]['rms_xy_jja_GLB']
            rms4 = var_cmip5_dics[var][mod]["defaultReference"][
                "r1i1p1"]["global"]['rms_xy_son_GLB']

            if P.verbose:
                print var, ' ', mod, '  ', repr(rms1), ' WITH global'
            ...
            if P.verbose:
                print var, ' ', mod, '  ', repr(rms1), ' missing'
    ...
    med_rms1 = statistics.median(vars_ar1)[0]
    ...
    for mod in mods:
        ...
        try:
            out1_rel[vn, mn] = (float(var_cmip5_dics[var][mod]["defaultReference"][
                                "r1i1p1"]["global"]['rms_xy_djf_GLB']) - med_rms1) / med_rms1  # RELATIVE ERROR
            out2_rel[vn, mn] = (float(var_cmip5_dics[var][mod]["defaultReference"][
                                "r1i1p1"]["global"]['rms_xy_mam_GLB']) - med_rms2) / med_rms2  # RELATIVE ERROR
            out3_rel[vn, mn] = (float(var_cmip5_dics[var][mod]["defaultReference"][
                                "r1i1p1"]["global"]['rms_xy_jja_GLB']) - med_rms3) / med_rms3  # RELATIVE ERROR
            out4_rel[vn, mn] = (float(var_cmip5_dics[var][mod]["defaultReference"][
                                "r1i1p1"]["global"]['rms_xy_son_GLB']) - med_rms4) / med_rms4  # RELATIVE ERROR
