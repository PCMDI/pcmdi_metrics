...
PJD 20 Jan 2015     - Updated to latest build
PJD 20 Jan 2015     - Copied GFDL demo file and edited paths
PJD 21 Jan 2015     - Updated to loop through CMORised variables
...
uvcdatInstall = ''.join(['/short/p66/pjd599/PCMDI_METRICS/',buildVersion,'/bin/'])
...
inVars          = [['pr','psl','rlut','rlutcs','rsut','rsutcs','ta','tas','ua','uas','va','vas','zg'],['sftlf'],['sos','tos'],]
inTimes         = [500,529] ; # Depending on the 'indicator' settings below the year 529 will be excluded (or included) from calculations
    ...
    for count2,experimentId in enumerate(inExperiments):
        for count3,realmId in enumerate(inRealms):
            for count4,varId in enumerate(inVars[count3]):
                ...
                if tableId == 'fx':
                    dataPath = '/g/data/ua6/drstree/CMIP5/GCM/CSIRO-BOM/_MODEL_/_EXPERIMENT_/_REALM_/atmos/_VAR_/r0i0p0/'
                else:
                    dataPath = inDataPath
                ...
                outFileXml = ''.join(['xmls/',varId,'_',modelId,'_',experimentId,'_',realmId,'.xml'])
                command = ''.join([uvcdatInstall,'cdscan -x ',outFileXml,' ',dataPath,varId,'*.nc'])
                #print command
                ...
                if realmId == 'fx':
                    ...
                    data = fIn[varId](time=(str(inTimes[0]),str(inTimes[-1]+1),'con')) ; # String specifies 'years' or componentTime - indexed are offset by one (within bounds)
                    #print data.shape
                    ...
                    #print data.shape
                inTimeStr = "-".join([''.join([(format(inTimes[0],'04d')),'01']),''.join([(format(inTimes[1],'04d')),'12'])])
                data.astype('float32')
                data.id = varId
                print "".join(['** Writing variable: ',varId,' **'])
                if realmId == 'fx':
                    outfile = ''.join(['ncs/','_'.join([varId,modelId,experimentId,tableId,'r0i0p0.nc'])])
                else:
                    #outfile = ncs/tas_ACCESS1-0_piControl_Amon_r1i1p1_050001-052912-clim.nc
                    outfile = ''.join(['ncs/','_'.join([varId,modelId,experimentId,tableId,'r1i1p1',inTimeStr]),'-clim.nc'])
