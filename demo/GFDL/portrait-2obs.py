...
try:
  ...
  raise RuntimeError("Sorry your python is not build with VCS support cannot genrate portrait plots")
...
#vars = ['pr','tas','psl','huss','uas','vas', 'rlut','rsut','rsutcs','rlutcs','rsds','rlus','ua-850','ua-200','va-850','va-200','zg-500']

vars = ['pr','prw','tas','psl','huss','rlut','ua-850','ua-200','va-850','va-200','zg-500']
...
json_files = glob.glob(os.path.join(sys.prefix,"share","CMIP_metrics_results","CMIP5","historical","*.json"))
json_files = glob.glob('/work/gleckler1/processed_data/metrics_package/metrics_results/cmip5clims_metrics_package-picontrol/cmip5_test/*.json')

#json_files_acme = glob.glob('/export/gleckler1/141126_metrics-acme/sampletest_141204/*.json')

json_files_gfdl = glob.glob('/work/durack1/Shared/140808_metrics-gfdl/sampletest/*.json')
...
vars = ['pr','tas','psl','uas','vas', 'rlut','rsut','rsutcs','rlutcs','ua-850','ua-200','va-850','va-200','zg-500']

vars = ['pr','tas','psl','rlut','ua-850','ua-200','va-850','va-200','zg-500']
...
P.plot(out1_rel,x=x,multiple=1.2,bg=1)
P.plot(out2_rel,x=x,multiple=2.2,bg=1)
...
### SAVE PLOT
#x.postscript('test')  # EXPECTED WITH UV-CDAT 2.1.1
x.png('test_2obs_xyt_RMS')
#x.pdf('test'   # EXPECTED WITH UV-CDAT 2.1.1)
