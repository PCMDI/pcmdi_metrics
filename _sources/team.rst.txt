.. title:: PMP Team

.. _team:

****
Team
****

Current Core Development Team
==============================

* `Jiwoo Lee <https://people.llnl.gov/lee1043>`_ (`LLNL`_, PMP Lead)
* `Kristin Chang <https://people.llnl.gov/chang61>`_ (`LLNL`_)
* `Seth Goodnight <https://people.llnl.gov/goodnight1>`_ (`LLNL`_)
* `Peter Gleckler <https://pcmdi.llnl.gov/staff/gleckler/>`_ (`LLNL`_, PMP Founder)
* `Paul Ullrich <https://people.llnl.gov/ullrich4>`_ (`LLNL`_, `PCMDI <https://pcmdi.llnl.gov/>`_ Project PI)
* `Shixuan Zhang <https://www.pnnl.gov/science/staff/staff_info.asp?staff_num=9376>`_ (`PNNL`_, `E3SM`_ POC)

**Contact:** pcmdi-metrics@llnl.gov

.. _LLNL: https://www.llnl.gov/
.. _PNNL: https://www.pnnl.gov/
.. _LANL: https://www.lanl.gov/
.. _NCAR: https://ncar.ucar.edu/
.. _NRL: https://www.nrl.navy.mil/
.. _E3SM: https://e3sm.org/

Key Contributors and Collaborators
===================================

MJO and Precipitation Benchmarking
-----------------------------------

* Min-Seop Ahn (NASA GSFC) — former team member

Ocean Metrics
-------------

* Paul Durack (former `LLNL`_ affiliated, now at Department of Water and Environmental Regulation, Australia)

ENSO Metrics
------------

* Yann Planton (CNRS, France)
* Eric Guilyardi (IPSL, France; Univ. of Reading, UK)

Cloud Feedback
--------------

* Mark Zelinka (`LLNL`_)
* Li-Wei Chao (former `LLNL`_ affiliated, now at Tufts University)

E3SM Workflow Integration
--------------------------

* Chengzhu (Jill) Zhang (`LLNL`_)
* Shixuan Zhang (`PNNL`_)

QBO Metrics
-----------

* Julie Caron (NCAR)

Modes of Variability
--------------------

* Shixuan Zhang (`PNNL`_)
* Martin Velez Pardo (`LANL`_)
* Alexandra Jonko (`LANL`_)

High Latitude Metrics
---------------------

* Younjoo Lee (`NRL`_)
* Wilbert Weijer (`LANL`_)
* Alexandra Jonko (`LANL`_)

Former Team Members
===================

* Ana Ordonez (PMP framework and metrics implementation, now at Eagle Rock Analytics)
* Bo Dong (PMP update)
* Charles Doutriaux (PMP framework, `LLNL`_)