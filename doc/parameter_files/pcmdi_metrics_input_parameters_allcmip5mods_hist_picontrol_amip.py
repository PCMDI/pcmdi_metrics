...
simulation_description_mapping = {
    "Login": "Login",
    "Center": "Center",
    "CMIP5CreationDate": "creation_date",
    "CMIP5_tracking_id": 'tracking_id'}
simulation_description_mapping = {
...
# filename_template = "cmip5.%(model_version).' + exp +
# '.r1i1p1.mo.%(table_realm).%(variable).ver-v20110601.1980-2005.AC.nc" ##
# tos

if exp == 'historical':
    # FOR AMON FIELDS
    filename_template = "%(variable)_%(model_version)_Amon_historical_%(exp)r1i1p1_198001-200512-clim.nc"
    # FOR OMON FIELDS
    filename_template = "%(variable)_%(model_version)_Omon_historical_%(exp)r1i1p1_198001-200512-clim.nc"
...
if exp == 'picontrol':
    ...
    # FOR OMON FIELDS
    filename_template = "%(variable)_%(model_version)_Omon_picontrol_%(exp)r1i1p1_01-12-clim.nc"
...
metrics_output_path = '/work/gleckler1/processed_data/metrics_package/metrics_results/cmip5clims_metrics_package-' + \
    exp
# DIRECTORY WHERE TO PUT INTERPOLATED MODELS' CLIMATOLOGIES
model_clims_interpolated_output = '/work/gleckler1/processed_data/metrics_package/interpolated_model_clims_' + \
    exp + '/'
# FILENAME FOR INTERPOLATED CLIMATOLOGIES OUTPUT
filename_output_template = "cmip5.%(model_version).historical.r1i1p1.mo." +\
    "%(table_realm).%(variable)%(level).ver-1.%(period)." +\
    "interpolated.%(regridMethod).%(targetGridName).AC%(ext)"
