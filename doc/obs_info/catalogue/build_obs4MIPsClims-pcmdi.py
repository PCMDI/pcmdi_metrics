...
if len(sys.argv) > 1:
    ...
    data_path = '/p/user_pub/pmp/pmp_reference/'

full_path = data_path + 'obs4MIPs_clims/' + vrs + '/gn/' + verin + '/*_' +sea + '.nc' 
...
for l in lsttt:
   ...
   by_source[source_id][vr]['PCMDI-path'] = path + '/'
   by_source[source_id][vr]['path'] = path.split(data_path)[1] + '/' 
   ...
   by_var[vr][source_id]['PCMDI-path'] = path + '/'
   by_var[vr][source_id]['path'] = path.split(data_path)[1] + '/' 
...
pths_out = [pathout,'./']
for pth in pths_out:
 with open(pth + "PMP-obs4MIPsClims-bySource-" + ver + ".json", "w") as outfile:
     outfile.write(json.dumps(by_source, indent=4))
 with open(pth + "PMP-obs4MIPsClims-byVar-" + ver + ".json", "w") as outfile:
