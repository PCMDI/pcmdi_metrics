...
vars = ['tas', 'psl', 'tauu', 'tauv', 'ta_850', 'ta_200', 'ua_850', 'ua_200', 'va_850', 'va_200', 'zg_500', 'pr', 'rltcre', 'rstcre', 'rt', 'rst', 'rlut']
...
if metrics_in_single_file == 'y':
  # All in same file
  metrics_output_path = results_dir + '/pmp_results/pmp_v1.1.2/metrics_results/mean_climate/cmip6/historical/%(case_id)/'
else:
  # Individual model files
  metrics_output_path = results_dir + '/pmp_results/pmp_v1.1.2/metrics_results/mean_climate/cmip6/historical/%(case_id)/%(variable)%(level)/'
...
if metrics_in_single_file == 'y':
  # All in same file
  output_json_template = '%(variable)%(level).cmip6.historical.%(regrid_method).' + target_grid_string + '.'  + case_id
else:
  # Individual model files
  output_json_template = '%(model_version).%(variable)%(level).cmip6.historical.%(regrid_method).' + target_grid_string + '.'  + case_id
