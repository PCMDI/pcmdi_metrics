...
try:
    ...
    raise RuntimeError(
        "Sorry your python is not build with VCS support and cannot generate portrait plots"
    )
...
P.PLOT_SETTINGS.x1 = 0.1
P.PLOT_SETTINGS.x2 = 0.85
P.PLOT_SETTINGS.y1 = 0.22
P.PLOT_SETTINGS.y2 = 0.95
...
json_files = glob.glob(
    os.path.join(
        sys.prefix, "share", "CMIP_metrics_results", "CMIP5", "piControl", "*.json"
    )
)
...
out1_rel, out2_rel, out3_rel = [
    np.ma.masked_all((len(vars), len(mods)), np.float32) for _ in range(3)
...
for vn, var in enumerate(vars):
    ...
    for mn, mod in enumerate(mods):
        try:
            out1_rel[vn, mn] = float(
                var_cmip5_dics[var][mod]["defaultReference"]["r1i1p1"]["global"][
                    "rms_xy_djf_GLB"
                ]
            )
            out2_rel[vn, mn] = float(
                var_cmip5_dics[var][mod]["alternate1"]["r1i1p1"]["global"][
                    "rms_xy_djf_GLB"
                ]
            )
            out3_rel[vn, mn] = float(
                var_cmip5_dics[var][mod]["alternate2"]["r1i1p1"]["global"][
                    "rms_xy_djf_GLB"
                ]
            )
...
# x.pdf(fileName) ; # Labelling requires VCS updates expected with UV-CDAT
# 2.1.1 release
