...
from pcmdi_metrics.drcdm.lib import compute_metrics
...
def create_random_precip(years, max_val=None, min_val=None):
    ...
    values = np.ones((len(times), latd, lond)) * 20 + np.random.randint(
        -10, high=10, size=(len(times), latd, lond)
    )
    ...
    fake_ds = xr.Dataset(
        {
            "pr": xr.DataArray(
                ...
                dims=["time", "lat", "lon"],
                coords={"time": times, "lat": lat, "lon": lon},
                attrs={"_FillValue": -999.9, "units": "mm"},
            )
        }
    )
...
def test_seasonal_averager_settings():
    ...
    ds, sftlf = create_random_precip([1980, 1981])
...
def test_seasonal_averager_ann_mean():
    drop_incomplete_djf = True
    dec_mode = "DJF"
    annual_strict = True
    ds, sftlf = create_random_precip([1980, 1981])
    ...
    ann_max = S.annual_stats("mean", pentad=False)

    assert np.mean(ann_max) == np.mean(ds.temporal.group_average("pr", "year"))
...
def test_seasonal_averager_ann_djf():
    ...
    assert djf.max().data == 1.0
    assert djf.mean().data == 1.0
