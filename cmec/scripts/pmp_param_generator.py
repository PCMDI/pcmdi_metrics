...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Error: could not read " + config_json + ". File might not be valid JSON."
        )
        sys.exit(1)
    ...
    if pmp_config in ["variability_modes", "mjo", "monsoon_sperber"]:
        ...
        if "reference_data_lf_path" in settings:
            settings["reference_data_lf_path"] = os.path.join(
                obs_dir, settings["reference_data_lf_path"]
            )
