...
if [[ $? = 0 ]]; then
    ...
    fourierDiurnalGridpoints.py -p $tmp_param --results_dir $CMEC_WK_DIR/ascii --modpath $CMEC_WK_DIR/nc -t 'pr_%(model)_%(month)_%(firstyear)-%(lastyear)_diurnal_avg.nc' --filename_template_std 'pr_%(model)_%(month)_%(firstyear)-%(lastyear)_diurnal_std.nc'

    printf "\nstd_of_hourlyvalues\n"
    std_of_hourlyvalues.py -p $tmp_param --results_dir $CMEC_WK_DIR/json --modpath $CMEC_WK_DIR/nc

    printf "\nstd_of_meandiurnalcycle"
    std_of_meandiurnalcycle.py -p $tmp_param --results_dir $CMEC_WK_DIR/json --modpath $CMEC_WK_DIR/nc

    printf "\nfourierDiurnalAllGrid\n"
    fourierDiurnalAllGrid.py -p $tmp_param --results_dir $CMEC_WK_DIR/nc --modpath $CMEC_WK_DIR/nc -t 'pr_%(model)_%(month)_%(firstyear)-%(lastyear)_diurnal_avg.nc'

    printf "\nsavg_fourier.py\n"
    savg_fourier.py -p $tmp_param --results_dir $CMEC_WK_DIR/json --modpath $CMEC_WK_DIR/nc -t 'pr_%(model)_%(month)_%(firstyear)-%(lastyear)_S.nc'
