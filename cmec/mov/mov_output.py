...
desc = {
    "Modes of Variability metrics": {
        ...
        "description": variability_mode
        + " statistics for different EOF modes in CMEC formatted JSON",
    },
...
for eof in ["1", "2", "3"]:
    for season in seasons:
        ...
        for mod in modnames:
            name_seg = "_".join(
                [variability_mode, varModel, "EOF" + eof, season, mip, mod]
            )
            if nc_out_model:
                result_file = [
                    f for f in nc_list if ((name_seg in f) and ("_cbf" not in f))
                ][0]
                tmp = {
                    "_".join([mod, "EOF" + eof, season]): {
                        "filename": result_file,
                        "long_name": " ".join(
                            [mod, season, variability_mode, "EOF", eof, "data"]
                        ),
                        "description": "Global map, PC timeseries, and fraction for "
                        + mod
                        + " EOF "
                        + eof,
                    }
                ...
                if result_file_tmp in nc_list:
                    tmp = {
                        "_".join([mod, "EOF" + eof, season, "cbf"]): {
                            "filename": result_file,
                            "long_name": " ".join(
                                [mod, season, variability_mode, "EOF", eof, "CBF data"]
                            ),
                            "description": "Common Basis Function approach global map, PC timeseries, and fraction for "
                            + mod
                            + " EOF "
                            + eof,
                        }
            ...
            if plot_model:
                ...
                tmp = {
                    "_".join([mod, "EOF" + eof, season]): {
                        "filename": result_file,
                        "long_name": " ".join(
                            [mod, season, variability_mode, "EOF", eof, "map"]
                        ),
                        "description": "Map of "
                        + season
                        + " "
                ...
                if result_file_tmp in png_list:
                    tmp = {
                        "_".join([mod, "EOF" + eof, season, "teleconnection"]): {
                            ...
                            "long_name": " ".join(
                                [mod, "EOF", eof, season, "teleconnection map"]
                            ),
                            "description": "Extended global teleconnection map for model "
                            + mod
                            + " EOF "
                            + eof,
                        }
                ...
                if result_file_tmp in png_list:
                    tmp = {
                        "_".join([mod, "EOF" + eof, season, "cbf"]): {
                            "filename": result_file_tmp,
                            "long_name": " ".join(
                                [mod, "EOF", eof, season, "cbf", "map"]
                            ),
                            "description": "Common basis function map for model "
                            + mod
                            + " EOF "
                            + eof,
                        }
                ...
                if result_file_tmp in png_list:
                    tmp = {
                        "_".join([mod, "EOF" + eof, season, "cbf teleconnection"]): {
                            "filename": result_file_tmp,
                            "long_name": " ".join(
                                [mod, "EOF", eof, season, "cbf teleconnection map"]
                            ),
                            "description": "Common basis function teleconnection map for model "
                            + mod
                            + " EOF "
                            + eof,
                        }
        ...
        if nc_out_obs:
            ...
            tmp = {
                "_".join([mod, mip]): {
                    "filename": result_file,
                    "long_name": " ".join(
                        ["obs", season, variability_mode, "EOF", eof, "data"]
                    ),
                    "description": "Map of "
                    + season
                    + " "
        ...
        if plot_obs:
            result_file = [
                f for f in png_list if mod in f and "teleconnection" not in f
            ][0]
            tmp = {
                "_".join([mod, mip]): {
                    "filename": result_file,
                    "long_name": " ".join(
                        ["obs", season, variability_mode, "EOF", eof, "map"]
                    ),
                    "description": "" + mod,
                }
            }
            ...
            if result_file_tmp in png_list:
                tmp = {
                    "_".join([mod, mip, "teleconnection"]): {
                        ...
                        "description": "Extended global teleconnection map for obs EOF "
                        + eof,
                    }
                }
                plots.update(tmp)
