...
def KT_decomposition_general(c1, c2, Klw, Ksw):
    ...
    Klw_resid_prime = (
        Klw_prime - np.expand_dims(Klw_p_prime, 0) - np.expand_dims(Klw_t_prime, 1)
    )  # Eq. B9
    dRlw_true = np.ma.sum(np.ma.sum(Klw * dc, 1), 0)  # LW total
    dRlw_prop = Klw0 * sum_dc  # LW amount component
    dRlw_dctp = np.ma.sum(
        Klw_p_prime * np.ma.sum(dc_star, 0), 0
    ...
    Ksw_resid_prime = (
        Ksw_prime - np.expand_dims(Ksw_p_prime, 0) - np.expand_dims(Ksw_t_prime, 1)
    )  # Eq. B9
    dRsw_true = np.ma.sum(np.ma.sum(Ksw * dc, 1), 0)  # SW total
    dRsw_prop = Ksw0 * sum_dc  # SW amount component
    dRsw_dctp = np.ma.sum(
        Ksw_p_prime * np.ma.sum(dc_star, 0), 0
