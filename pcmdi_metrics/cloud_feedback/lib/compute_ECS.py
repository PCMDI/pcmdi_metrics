...
def get_anom_abrupt(ab_path, pi_path, var):
    ...
    FN = r.add(
        12 * 150, cdtime.Months
    )  # add 121 rather than 120 because it uses day 1 rather than day 15
    ab_fn = FN.value
    ...
    true_fn = r.add(
        15, cdtime.Days
    ).value  # add 15 days on to take it to the end of the month
    ...
    extend_fn = r.add(
        121, cdtime.Months
    ).value  # add 121 rather than 120 because it uses day 1 rather than day 15
