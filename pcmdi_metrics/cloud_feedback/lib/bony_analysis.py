...
def bony_sorting_part2(OKwaps, data, OKland, WTS, binedges):
    ...
    sh = list()
    sh = np.append(
        data.shape[0], 3 + len(binedges)
    )  # add 2 for the exceedances, 1 for land
    ...
    A1b = np.moveaxis(
        np.moveaxis(binary_waps2 * WTS, 0, -1), 0, -1
    ...
    for xx in range(2 + len(binedges)):
        A2 = (
            binary_data2 * A1b[..., xx, :]
        )  # zero for wap outside range or undefined data, frac area for wap inside range
    ...
    if np.allclose(0.5, np.sum(CNTS, 0)) is False:
        print(
    ...
    if np.allclose(v1, v2a) is False or np.allclose(v1, v2b) is False:
