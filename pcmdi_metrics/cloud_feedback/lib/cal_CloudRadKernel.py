...
TR = cdutil.region.domain(latitude=(-30.0, 30.0))
...
def apply_land_mask_v2(data):
    ...
    land_data = MV.masked_where(mask2 == 0.0, data)
...
def get_area_wts(data):
    ...
    area_wts = MV.array(
        np.moveaxis(np.tile(coslat2 / C, [A, C, 1]), 1, 2)
...
def get_CRK_data(filenames):
    ...
    if np.max(sumclisccp1) <= 1.0:
        ctl_clisccp = ctl_clisccp * 100.0
        fut_clisccp = fut_clisccp * 100.0
    ...
    ctl_albcs = MV.where(ctl_albcs < 0.0, 0, ctl_albcs)
    ...
    LWkernel_map = np.tile(
        np.tile(LWkernel[:, :, :, :, 0], (1, 1, 1, 1, 1)), (144, 1, 1, 1, 1)
    )(order=[1, 2, 3, 4, 0])
    ...
    avgdtas0 = cdutil.averager(
        anom_tas, axis="xy", weights="weighted"
    )  # global average
    ...
    ctl_clisccp_wap, ctl_N = BA.bony_sorting_part2(
        ctl_OKwaps, TCC, ctl_wap_land, WTS, binedges
    )
    fut_clisccp_wap, fut_N = BA.bony_sorting_part2(
        fut_OKwaps, TFC, fut_wap_land, WTS, binedges
    )
    LWK_wap, P_wapbin = BA.bony_sorting_part2(
        ctl_OKwaps, TLK, ctl_wap_land, WTS, binedges
    )
    SWK_wap, P_wapbin = BA.bony_sorting_part2(
        ctl_OKwaps, TSK, ctl_wap_land, WTS, binedges
    )
...
def get_plottable_time(X):
    ...
    years = [x.year + (x.month - 1) / 12.0 for x in X.getTime().asComponentTime()]
...
def klein_metrics(obs_clisccp, gcm_clisccp, LWkern, SWkern, WTS):
    ...
    if np.ndim(WTS_dom) == 2:  # working in wap space
        ...
        avg_obs_clisccp_dom = np.ma.sum(
            np.ma.sum(np.ma.sum(this * WTS_dom, -1), -1), -1
        )  # (TAU,CTP)
    ...
    for Tsec in Tsec_name:
        ...
        for Psec in Psec_name:
            ...
            if len(CTPmids[PP]) > 0:
                pp += 1
                agg_obs_NET_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(obs_NET_bias)[:, TT, PP, :], 1), 1
                )
                agg_gcm_NET_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(gcm_NET_bias)[:, TT, PP, :], 1), 1
                )
                agg_obs_SW_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(obs_SW_bias)[:, TT, PP, :], 1), 1
                )
                agg_gcm_SW_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(gcm_SW_bias)[:, TT, PP, :], 1), 1
                )
                agg_obs_LW_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(obs_LW_bias)[:, TT, PP, :], 1), 1
                )
                agg_gcm_LW_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(gcm_LW_bias)[:, TT, PP, :], 1), 1
                )
                agg_obs_clisccp_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(anom_obs_clisccp_dom)[:, TT, PP, :], 1), 1
                )
                agg_gcm_clisccp_bias[:, tt, pp, :] = np.ma.sum(
                    np.ma.sum(np.array(clisccp_bias_dom)[:, TT, PP, :], 1), 1
                )
    NP = pp + 1
    NT = tt + 1
...
def obscuration_terms3(c1, c2):
    ...
    U12 = MV.sum(MV.sum(c12[:, :, 2:, :], 1), 1) / 100.0
        ...
        for sfc in surfaces:
            ...
            for sec in sections:
                ...
                for name in names:
                    ...
                    if sfc == "ocn":
                        wtd_fbk = np.ma.sum(
                            np.ma.sum(domain.select(ocn_fbk * area_wts), 1), 1
                        )
                        if norm:
                            denom = np.ma.sum(
                                np.ma.sum(domain.select(ocn_area_wts), 1), 1
                            )
                    elif sfc == "lnd":
                        wtd_fbk = np.ma.sum(
                            np.ma.sum(domain.select(lnd_fbk * area_wts), 1), 1
                        )
                        if norm:
                            denom = np.ma.sum(
                                np.ma.sum(domain.select(lnd_area_wts), 1), 1
                            )
                    elif sfc == "all":
                        wtd_fbk = np.ma.sum(
                            np.ma.sum(domain.select(fbk * area_wts), 1), 1
                        )
                        if norm:
...
def CloudRadKernel(filenames):
    ...
    for sec in sections:
        for name in names:
            fbk_dict["eq30"]["ocn_asc"][sec][name] = np.ma.average(
                np.sum((output_wap[sec][name])[:, :cutoff], 1), 0
            )
            fbk_dict["eq30"]["ocn_dsc"][sec][name] = np.ma.average(
                np.sum((output_wap[sec][name])[:, cutoff:], 1), 0
            )
    ...
    for name in names:
        obsc_fbk_dict["eq30"]["ocn_asc"][sec][name] = np.ma.average(
            np.sum((obsc_output_wap[sec][name])[:, :cutoff], 1), 0
        )
        obsc_fbk_dict["eq30"]["ocn_dsc"][sec][name] = np.ma.average(
            np.sum((obsc_output_wap[sec][name])[:, cutoff:], 1), 0
        )
