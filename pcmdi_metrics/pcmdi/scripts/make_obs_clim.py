...
parser.add_argument(
    "file", metavar="str", type=str, help="'file' will select one file to process"
)
parser.add_argument(
    "file_variable",
    metavar="str",
...
parser.add_argument(
    ...
    metavar="str",
    type=str,
    help="'data_source' will use this descriptor in outfile name creation",
)
parser.add_argument(
    "start_yr",
    nargs="?",
    metavar="int",
    type=int,
    help="'start_yr' set a start year from which to process(default=1980)",
)
parser.add_argument(
    "end_yr",
    nargs="?",
...
if args.target_variable == "sftlf":
    ...
    clim_ac = cdu.ANNUALCYCLE.climatology(
        d(
            time=(
                cdt.comptime(int(start_yr_s), 1, 1),
                cdt.comptime(int(end_yr_s) + 1, 1, 1),
                boundnodes,
            )
        )
    )  # shape 12,180,360
    ...
    tmp = d(
        time=(
            cdt.comptime(int(start_yr_s), 1, 1),
            cdt.comptime((int(start_yr_s) + 1), 1, 1),
            boundnodes,
        )
    )
    t = tmp.getAxis(0)
    ...
    [
        "_".join(
            [
                ...
                "-".join(
                    [
                        "".join([start_yr_s, start_month_s]),
                        "".join([end_yr_s, end_month_s]),
                        "clim",
                    ]
                ),
            ]
...
f_out.pcmdi_metrics_comment = (
    "This climatology was prepared by PCMDI "
