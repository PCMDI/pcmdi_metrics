...
def tgrid(t):
 ...
 time=t[:]*0.
 if t[0]==0. : dt=0.
 else : dt=0.5		# centered in the midlle of the month
 time[0]=dt/12.
 nmonths=len(t) 	# monthy time series

 for it in range (1,nmonths) :
    time[it]=time[it-1]+1./12.
...
# Creating regional masks
    lons_p=MV.where(MV.less(lons_n,0.),lons_n+360.,lons_n)
    lons_a=lons_n
...
# Arctic Regions
#Central Arctic
    lat_bound1=MV.logical_and(MV.greater(lats_n,80.),MV.less_equal(lats_n,87.2))
    lat_bound2=MV.logical_and(MV.greater(lats_n,65.),MV.less_equal(lats_n,87.2))
    lat_bound3=MV.logical_and(MV.greater(lats_n,87.2),MV.less_equal(lats_n,90.))
...
#NA region
    lat_bound=MV.logical_and(MV.greater(lats_n,35.),MV.less_equal(lats_n,80.))
...
#NP region
    lat_bound=MV.logical_and(MV.greater(lats_n,35.),MV.less_equal(lats_n,65.))
    ...
    area_sic_pole=MV.where(MV.equal(mask_pole,True),MV.multiply(1,area_n),0.)

#Masking out sic<0.15
    ice_area = MV.where(MV.greater_equal(sic_n,0.15),area_n,0.)                     #Masking out sic<0.15
...
# Ice Area
    ...
    area_sic_ca=MV.where(MV.equal(mask_ca,True),MV.multiply(sic_n,ice_area),0.)
    area_sic_na=MV.where(MV.equal(mask_na,True),MV.multiply(sic_n,ice_area),0.)
    area_sic_np=MV.where(MV.equal(mask_np,True),MV.multiply(sic_n,ice_area),0.)
...
# Creating regional masks
    lons_sa=lons_s
    lons_sp=MV.where(MV.less(lons_s,0.),lons_s+360,lons_s)
    lons_io=lons_sp
...
# Antarctic Regions
    lat_bound=MV.logical_and(MV.greater(lats_s,-90.),MV.less_equal(lats_s,-40.))

# SA region
#    lon_bound=MV.logical_and(MV.greater(lons_sa,-60.),MV.less_equal(lons_sa,30.))
    lon_bound=MV.logical_and(MV.greater(lons_sa,-60.),MV.less_equal(lons_sa,20.))
...
#    lon_bound=MV.logical_and(MV.greater(lons_sp,130.),MV.less_equal(lons_sp,300.))
    lon_bound=MV.logical_and(MV.greater(lons_sp,90.),MV.less_equal(lons_sp,300.))
...
#    lon_bound=MV.logical_and(MV.greater(lons_sp,30.),MV.less_equal(lons_sp,130.))
    lon_bound=MV.logical_and(MV.greater(lons_sp,20.),MV.less_equal(lons_sp,90.))
    mask_io=MV.where(MV.logical_and(lat_bound,lon_bound),1,0)

#Masking out sic<0.15
    ice_area = MV.where(MV.greater_equal(sic_s,dc),area_s,0.)                     #Masking out sic<0.15
# Ice Extent
...
# Ice Area
    ...
    area_sic_sa=MV.where(MV.equal(mask_sa,True),MV.multiply(sic_s,area_s),0.)
    area_sic_sp=MV.where(MV.equal(mask_sp,True),MV.multiply(sic_s,area_s),0.)
    area_sic_io=MV.where(MV.equal(mask_io,True),MV.multiply(sic_s,area_s),0.)
  ...
  obs_s=MV.masked_equal(obs_s,-9999.)
...
for im in range (0,12):
    annual_cycle_std_obs_arctic[im,:]=np.array(genutil.statistics.std(obs_n[im:324:12,:]))
    annual_cycle_std_obs_ca[im,:]=np.array(genutil.statistics.std(obs_ca[im:324:12,:]))
    annual_cycle_std_obs_na[im,:]=np.array(genutil.statistics.std(obs_na[im:324:12,:]))
    annual_cycle_std_obs_np[im,:]=np.array(genutil.statistics.std(obs_np[im:324:12,:]))
    annual_cycle_std_obs_antarctic[im,:]=np.array(genutil.statistics.std(obs_s[im:324:12,:]))
    annual_cycle_std_obs_sa[im,:]=np.array(genutil.statistics.std(obs_sa[im:324:12,:]))
    annual_cycle_std_obs_sp[im,:]=np.array(genutil.statistics.std(obs_sp[im:324:12,:]))
    annual_cycle_std_obs_io[im,:]=np.array(genutil.statistics.std(obs_io[im:324:12,:]))


# NSIDC-0192
# SSM/I Arctic
# Area
dlist_n=['nasateam/gsfc.nasateam.month.area.1978-2010.n.asc','bootstrap/gsfc.bootstrap.month.area.1978-2010.n.asc']
dlist_s=['nasateam/gsfc.nasateam.month.area.1978-2010.s.asc','bootstrap/gsfc.bootstrap.month.area.1978-2010.s.asc']
# Extent
...
obs1_n=MV.masked_equal(obs1_n,-999.)
obs1_ca=MV.masked_equal(obs1_ca,-999.)
obs1_na=MV.masked_equal(obs1_na,-999.)
obs1_np=MV.masked_equal(obs1_np,-999.)
obs1_s=MV.masked_equal(obs1_s,-999.)
obs1_sa=MV.masked_equal(obs1_sa,-999.)
obs1_sp=MV.masked_equal(obs1_sp,-999.)
obs1_io=MV.masked_equal(obs1_io,-999.)
...
for im in range (0,12):
    annual_cycle_std_obs1_arctic[im,:]=np.array(genutil.statistics.std(obs1_n[im:324:12,:]))
    annual_cycle_std_obs1_ca[im,:]=np.array(genutil.statistics.std(obs1_ca[im:324:12,:]))
    annual_cycle_std_obs1_na[im,:]=np.array(genutil.statistics.std(obs1_na[im:324:12,:]))
    annual_cycle_std_obs1_np[im,:]=np.array(genutil.statistics.std(obs1_np[im:324:12,:]))
    annual_cycle_std_obs1_antarctic[im,:]=np.array(genutil.statistics.std(obs1_s[im:324:12,:]))
    annual_cycle_std_obs1_sa[im,:]=np.array(genutil.statistics.std(obs1_sa[im:324:12,:]))
    annual_cycle_std_obs1_sp[im,:]=np.array(genutil.statistics.std(obs1_sp[im:324:12,:]))
    annual_cycle_std_obs1_io[im,:]=np.array(genutil.statistics.std(obs1_io[im:324:12,:]))

# Calculate the Obs RMS
rms_arctic_obs1=genutil.statistics.rms(annual_cycle_obs1_arctic[:,1],annual_cycle_obs1_arctic[:,0],axis=0)
rms_antarctic_obs1=genutil.statistics.rms(annual_cycle_obs1_antarctic[:,1],annual_cycle_obs1_antarctic[:,0],axis=0)
rms_ca_obs1=genutil.statistics.rms(annual_cycle_obs1_ca[:,1],annual_cycle_obs1_ca[:,0],axis=0)
rms_na_obs1=genutil.statistics.rms(annual_cycle_obs1_na[:,1],annual_cycle_obs1_na[:,0],axis=0)
rms_np_obs1=genutil.statistics.rms(annual_cycle_obs1_np[:,1],annual_cycle_obs1_np[:,0],axis=0)
rms_sa_obs1=genutil.statistics.rms(annual_cycle_obs1_sa[:,1],annual_cycle_obs1_sa[:,0],axis=0)
rms_sp_obs1=genutil.statistics.rms(annual_cycle_obs1_sp[:,1],annual_cycle_obs1_sp[:,0],axis=0)
rms_io_obs1=genutil.statistics.rms(annual_cycle_obs1_io[:,1],annual_cycle_obs1_io[:,0],axis=0)
...
lines_m = ['ro-','rd-','ro--','co-','c*-','cd-','c-','bo-','b^-','b*-','co--','go--','g^--','gd--','g-','m*--','y*-'] #,'m^-','yo--']
#lines_d = ['bo-','g*-']
#cols_d=['b','g']
...
# Reading the sea ice concentration (sic)
    infile='/work/cmip5/historical/seaIce/mo/sic/' + 'cmip5.'+mod+'.historical.'+runs[ir]+'.mo.seaIce.sic.'+vers[ir]+'.xml'
    print infile

    f=cdms.open(infile)
    if (mod == 'HadGEM2-CC' and (runs[ir]=='r1i1p1' or runs[ir]=='r3i1p1')) or mod == 'HadGEM2-ES' and (runs[ir]=='r2i1p1'or runs[ir]=='r3i1p1'or runs[ir]=='r4i1p1') :
...
#NA region
    lat_bound=MV.logical_and(MV.greater(lats,35.),MV.less_equal(lats,80.))
    lon_bound=MV.logical_and(MV.greater(lons_a,-120.),MV.less_equal(lons_a,90.))
    mask_na=MV.where(MV.logical_and(lat_bound,lon_bound),1,0)

#NP region
    lat_bound=MV.logical_and(MV.greater(lats,35.),MV.less_equal(lats,65.))
    lon_bound=MV.logical_and(MV.greater(lons_p,90.),MV.less_equal(lons_p,240.))
    mask_np=MV.where(MV.logical_and(lat_bound,lon_bound),1,0)

#Antarctic Regions
    lat_bound=MV.logical_and(MV.greater(lats,-90.),MV.less_equal(lats,-40.))
...
#    arctic=MV.logical_and(MV.greater_equal(lats,35.),MV.less(lats,87.2))         #SSM/I limited to 87.2N
    arctic=MV.logical_and(MV.greater_equal(lats,35.),MV.less(lats,90.))         #Adding currently in SSM/I 100% in the area >87.2N
    antarctic=MV.logical_and(MV.greater_equal(lats,-90.),MV.less(lats,-40.))

    for nt in range (len(t)):
        aice_arctic=MV.where(MV.equal(arctic,True),ice_area[nt],0.)
        aice_antarctic=MV.where(MV.equal(antarctic,True),ice_area[nt],0.)
        area_sic_ca=MV.where(MV.equal(mask_ca,True),ice_area[nt],0.)
        area_sic_na=MV.where(MV.equal(mask_na,True),ice_area[nt],0.)
        area_sic_np=MV.where(MV.equal(mask_np,True),ice_area[nt],0.)
        area_sic_sa=MV.where(MV.equal(mask_sa,True),ice_area[nt],0.)
        area_sic_sp=MV.where(MV.equal(mask_sp,True),ice_area[nt],0.)
        area_sic_io=MV.where(MV.equal(mask_io,True),ice_area[nt],0.)
...
# Calculating the CMIP5 RMS
  for j in range (0,2) :
       rms_ann_arctic[j,i]=genutil.statistics.rms(ann_arctic[:,i],annual_cycle_obs_arctic[:,j],axis=0)
       rms_ann_antarctic[j,i]=genutil.statistics.rms(ann_antarctic[:,i],annual_cycle_obs_antarctic[:,j],axis=0)
       rms_ann_ca[j,i]=genutil.statistics.rms(ann_ca[:,i],annual_cycle_obs_ca[:,j],axis=0)
       rms_ann_na[j,i]=genutil.statistics.rms(ann_na[:,i],annual_cycle_obs_na[:,j],axis=0)
       rms_ann_np[j,i]=genutil.statistics.rms(ann_np[:,i],annual_cycle_obs_np[:,j],axis=0)
       rms_ann_sa[j,i]=genutil.statistics.rms(ann_sa[:,i],annual_cycle_obs_sa[:,j],axis=0)
       rms_ann_sp[j,i]=genutil.statistics.rms(ann_sp[:,i],annual_cycle_obs_sp[:,j],axis=0)
       rms_ann_io[j,i]=genutil.statistics.rms(ann_io[:,i],annual_cycle_obs_io[:,j],axis=0)
...
for im in range (0,12):
    annual_cycle_std_mod_arctic[im]=np.array(genutil.statistics.std(ttta_std_arctic[im,:]))
    annual_cycle_std_mod_antarctic[im]=np.array(genutil.statistics.std(ttta_std_antarctic[im,:]))
...
labels=["ACCESS1-3","BNU-ESM","CCSM4","CESM1-BGC","CESM1-CAM5-1-FV2","CESM1-CAM5","CESM1-FASTCHEM","CNRM-CM5","CSIRO-Mk3-6-0","CanCM4","CanESM2","GFDL-CM2p1","GFDL-CM3","GFDL-ES\
M2G","GFDL-ESM2M","GISS-E2-H-CC","GISS-E2-H","GISS-E2-R-CC","GISS-E2-R","HadCM3","HadGEM2-AO","HadGEM2-CC","HadGEM2-ES","IPSL-CM5A-MR","IPSL-CM5B-LR","MIROC-ESM-CHEM","MI\
ROC-ESM","MIROC4h","MIROC5","MPI-ESM-LR","MPI-ESM-MR","MPI-ESM-P","NorESM1-ME","bcc-csm1-1-m","bcc-csm1-1"]
...
plt.xticks(ind+width/2.,mlabels,rotation=20,size=8)
...
plt.annotate('North Atlantic Arctic Sector', (0.4, 0.9), xycoords='axes fraction',size=15)
plt.grid(True)
plt.hold
...
plt.annotate('North Pacific Arctic Sector', (0.4, 0.9), xycoords='axes fraction',size=15)
plt.grid(True)
...
plt.annotate('South Atlantic Ocean Antarctic Sector', (0.4, 0.9), xycoords='axes fraction',size=15)
plt.grid(True)
...
plt.annotate('South Pacific Ocean Antarctic Sector', (0.4, 0.9), xycoords='axes fraction',size=15)
...
plt.ylabel('RMS of Total Sea Ice Area, 10${^6}$km${^2}$')
plt.annotate('South Indian Ocean Antarctic Sector', (0.4, 0.9), xycoords='axes fraction',size=15)
