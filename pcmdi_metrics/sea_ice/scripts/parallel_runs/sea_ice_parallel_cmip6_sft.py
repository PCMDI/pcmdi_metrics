# Runs CMIP6 and searches for land/sea masks
...
sftof = xs.findPaths("historical", "sftof", "fx", cmipTable="Ofx", deduplicate=False)
...
# model_list = [x for x in model_list if x != "EC-Earth3-Veg"]
model_list = [x for x in model_list if x != "IPSL-CM6A-LR"]
print(model_list)
for model in ["EC-Earth3"]:
    ...
    try:
        ...
        print("No external variables for", model)
    ...
    try:
        ...
        if len(sfpath) > 0:
            sfpath = [tmp for tmp in sfpath if tmp.split("/")[-3] == grid]
            sfbase = os.path.basename(glob.glob(os.path.join(sfpath[0], "*")))
            if len(sfbase) > 0:
                sfbase = sfbase[-1]
                sf_path = os.path.join(sfpath[0], sfbase)
                gen_sft = False
            else:
                print("sftlf not found", model)
                print(dir_template)
                print(file_template)
        else:
