...
def merge_jsons(mip, exp, case_id, metricsCollection, pmprdir):
    ...
    json_file_template = "_".join(
        [
            "%(mip)_%(exp)_%(metricsCollection)",
            "%(case_id)",
            "%(model)",
            "%(realization)",
        ]
    ...
    json_file_template = (
        "%(mip)_%(exp)_%(metricsCollection)_%(case_id)_%(model)_%(realization)"
    )
...
def find_latest(pmprdir, mip, exp, MC):
    versions = sorted(
        [
            ...
            for r in glob.glob(
                os.path.join(
                    pmprdir, "metrics_results", "enso_metric", mip, exp, "v????????", MC
                )
            )
        ]
    )
