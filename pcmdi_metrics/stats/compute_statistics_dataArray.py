...
def calculate_temporal_correlation(
    data_array1: xr.DataArray, data_array2: xr.DataArray, dim: Union[str, tuple] = None
) -> float:
...
def calculate_spatial_correlation(
    ...
    wtarray: xr.DataArray = None,
    dim: Union[str, tuple] = None,
) -> float:
    return _calculate_correlation(
        data_array1, data_array2, method="spatial", wtarray=wtarray, dim=None
    )
    ...
    if method == "temporal":
        ...
        if dim is None:
            ...
            if dim not in data_array1.dims or dim not in data_array2.dims:
                raise ValueError(
                    f"Temporal dimension '{dim}' not found in both input DataArrays."
                )
        ...
        if dim is None:
            ...
            dim = (lat_key, lon_key)
            if (not isinstance(dim, tuple) and dim not in data_array1.dims) or (
                isinstance(dim, tuple)
                and any(
                    d not in data_array1.dims or d not in data_array2.dims for d in dim
                )
            ):
                raise ValueError(
                    f"Spatial dimension '{dim}' not found in both input DataArrays."
                )
