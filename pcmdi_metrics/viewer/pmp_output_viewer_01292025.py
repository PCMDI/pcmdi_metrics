...
# ----------
# Support II functions
# ----------

def create_mean_clim_divedown_dict(plot_path, season_map={'AC':'ANN'}):
    data = {}
        ...
        for file in files:
            if file.endswith(('png')):
                ...
                if len(parts) >= 8:
                    ...
                    region = parts[5]
                    season = parts[6]
                    mip = 'cmip6' # FIX LATER - add to params

                    # rename season if mapping provided
                    season = season_map.get(season, season)
                else:
                    variable, model, region, season, mip = None, None, None, None, None
...
def create_mean_clim_portrait_dict(plot_path):
    # rms_xy_global_portrait_mean_climate.png
    data = {}
    ...
    for this_path, this_dir, files in os.walk(plot_path):
        for file in files:
            if file.endswith(('.png')):
                parts = file.split('_')
                if len(parts) >= 4:
                    variable = parts[0] + '_' + parts[1]
                    region = parts[2]
                else:
                    variable, region = None, None
                ...
                if key not in data:
                    data[key] = {
                        'Variable': variable,
                        'Region': region,
                        'Portrait Plot': file_path
                    }
    return data

def create_bokeh_df(dict_function, plot_path, season_map={'AC':'ANN'}):
    
    data = dict_function(plot_path, season_map)
...
def create_bokeh_table(
    ...
    dtable = DataTable(source=filtered_source, columns=bokeh_columns, width=width, height=height, row_height=row_height)
    ...
    for fc in range(0, len(filter_columns)):
        ...
        if filter_column_name == 'Region':
            filter_widget_dict["dropdown" + str(fc)] = MultiChoice(
                options=list(df[filter_column_name].unique()),
...
def create_mean_clim_divedown_layout(plot_path, season_map={'AC':'ANN'}, compare_cmip6=False):
    ...
    """
    if plot_path:
        user_df = create_bokeh_df(create_mean_clim_divedown_dict, plot_path, season_map).sort_values(['MIP', 'Model', 'Variable', 'Region'])
    else:
    ...
    ordered_cols = ['MIP', 'Model', 'Variable', 'Description', 'Region', 'ANN', 'DJF', 'MAM', 'JJA', 'SON']
    df = df[ordered_cols]
    ...
    dtable = create_bokeh_table(
        ...
        image_columns=seasons,
        season_column_names=True
        )

    filter_columns = ['MIP', 'Model', 'Variable', 'Region']
    filter_widget_dict = create_bokeh_widgets(df, filter_columns=filter_columns)
    
    mip_dropdown = filter_widget_dict['dropdown0']
    model_dropdown = filter_widget_dict['dropdown1']
    var_dropdown = filter_widget_dict['dropdown2']
    region_dropdown = filter_widget_dict['dropdown3']
...
def create_mean_clim_portrait_layout(plot_path):
    if plot_path:
        user_df = create_bokeh_df(create_mean_clim_portrait_dict, plot_path)
    else:
        user_df = pd.DataFrame()
