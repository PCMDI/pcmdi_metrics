...
def view_pmp_output(
    ...
    for plot_path in plot_paths:
        if check_png_files(plot_path):
            summary_name = plot_path.split('/')[-5] 
            plot_name = os.path.basename(plot_path)
            layout = create_layout(plot_path=plot_path, season_map=season_map, compare_cmip6=compare_cmip6)
...
def create_layout(plot_path, season_map={'AC':'ANN'}, compare_cmip6=False):
    """
    ...
    layout = column(mip_dropdown, model_dropdown, var_dropdown, region_dropdown, dtable)
    return layout