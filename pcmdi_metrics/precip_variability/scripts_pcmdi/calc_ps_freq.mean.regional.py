            ...
            for iv, var in enumerate(vars):
                ...
                for frq in frqs:
                    ...
                    if (frq=='semi-annual'): # 180day=<pr=<183day
                        ...
                        fm = power.isel({"frequency":slice(idx1,idx2+1)}).mean("frequency").data
                        ...
                        fm = power.isel({"frequency":slice(0,idx1+1)}).mean("frequency").data
                        ...
                        fm = power.isel({"frequency":slice(idx1+1,idx2+1)}).mean("frequency").data
                        ...
                        if hr=='day':
                            ...
                            idx2=prdday_to_frq1didx(365,frequency)
                        fm = power.isel({"frequency":slice(0,idx2+1)}).mean("frequency").data
...
if not(os.path.isdir(outdir)):
    os.makedirs(outdir)
outfile=open(os.path.join(outdir,'PS_pr.{0}_regrid.{1}_freq.mean.pkl'.format(hr,res)),'wb')
