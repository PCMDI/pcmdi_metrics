...
from pcmdi_metrics.mean_climate.lib_driver.dataset import DataSet
...
class OutputMetrics(object):
    def __init__(self, parameter, var_name_long, obs_dict, sftlf):
        ...
        self.out_file = Base(
            self.parameter.metrics_output_path, self.parameter.output_json_template
        )
    ...
    def calculate_and_output_metrics(self, ref, test):
        ...
        if isinstance(self.obs_dict[self.var][ref.obs_or_model], basestring):
            self.obs_var_ref = self.obs_dict[self.var][
                self.obs_dict[self.var][ref.obs_or_model]
            ]
        else:
        ...
        parameter_realization = self.metrics_dictionary["RESULTS"][test.obs_or_model][
            ref.obs_or_model
    ...
    def set_simulation_desc(self, test, test_data):
        ...
        self.metrics_dictionary["RESULTS"][test.obs_or_model] = self.metrics_dictionary[
            "RESULTS"
        ].get(test.obs_or_model, {})
        if (
            ...
            self.metrics_dictionary["RESULTS"][test.obs_or_model]["units"] = getattr(
                test_data, "units", "N/A"
            )
    ...
    def output_interpolated_model_climatologies(self, test, test_data):
        ...
        pth = os.path.join(self.parameter.test_clims_interpolated_output, region_name)
    ...
    def check_save_test_clim(self, ref):
        ...
        return (
            not self.parameter.dry_run
            and hasattr(self.parameter, "save_test_clims")
            and self.parameter.save_test_clims is True
            and ref.obs_or_model == reference_data_set[0]
        )  # noqa
    ...
    def write_on_exit(self, cmec_flag):
        ...
        if not self.parameter.dry_run:
            logging.getLogger("pcmdi_metrics").info(
                "Saving results to: %s" % self.out_file()
            )
            self.out_file.write(
                self.metrics_dictionary,
                json_structure=[
            ...
            if cmec_flag is True:
                logging.getLogger("pcmdi_metrics").info(
                    "CMEC conversion: %s"
                    % self.out_file().replace(".json", "_cmec.json")
                )
