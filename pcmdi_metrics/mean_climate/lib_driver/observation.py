...
class OBS(Base):
    ...
    def __init__(self, root, var, obs_dict, obs="default", file_mask_template=None):
        ...
        template = obs_dict[var][obs_name].get(
            "template",
...
class Observation(DataSet):
    ...
    def __init__(
        self, parameter, var_name_long, region, obs, obs_dict, data_path, sftlf
    ):
    ...
    def create_obs_mask_name(self):
        ...
        try:
            ...
            obs_mask = OBS(
                self.data_path, "sftlf", self.obs_dict, obs_from_obs_dict["RefName"]
            )
    ...
    def get_obs_from_obs_dict(self):
        ...
        if isinstance(self.obs_dict[self.var][self.obs_or_model], basestring):
            obs_from_obs_dict = self.obs_dict[self.var][
                self.obs_dict[self.var][self.obs_or_model]
            ]
        else:
