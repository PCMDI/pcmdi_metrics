...
# all_mods_dic= json.load(open('all_mip_mods-v20220927_2.json'))  #all_mip_mods-v20200528.json'))
all_mods_dic = json.load(open('all_mip_mods-v20220928.json'))  #all_mip_mods-v20200528.json'))
...
vars = ['ts', 'psl', 'tauu', 'tauv', 'tas', 'ta_850', 'ta_200', 'ua_850', 'ua_200', 'va_850', 'va_200', 'zg_500', 'pr', 'rltcre', 'rstcre', 'rt', 'rst', 'rlut']
...
if regional == regional:  # 'y':
    regions_specs = {
        "Nino34": {"value": 0., "domain": cdutil.region.domain(latitude=(-5., 5.), longitude=(190., 240.))},
...
verd = '*'
if exp == 'amip':
    filename_template = "%(variable)_%(model_version)_%(table)_amip_%(exp)r1i1p1_198101-200512-clim.nc"
if exp == 'amip':
    filename_template = "CMIP5.amip.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.v20190225.nc"
# if exp == 'historical':
    # filename_template = "CMIP5.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.v20190307.nc"
if exp == 'historical' and MIP == 'cmip5':
    filename_template = MIP + '.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'amip' and MIP == 'cmip5':
    filename_template = MIP + '.amip.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'historical' and MIP == 'cmip6':
    filename_template = MIP + '.historical.%(model_version).r1i1p1f1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'amip' and MIP == 'cmip6':
    filename_template = MIP + '.amip.%(model_version).r1i1p1f1.mon.%(variable).198101-200512.AC.' + modver + '.nc'

# if exp == 'historical':
    # filename_template = "CMIP5.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.%('*').nc"
if exp == 'picontrol':
    filename_template = "%(variable)_%(model_version)_%(table)_picontrol_%(exp)r1i1p1_01-12-clim.nc"
...
# custom_observations = os.path.abspath('/p/user_pub/PCMDIobs/catalogue/obs4MIPs_PCMDI_clims_byVar_catalogue_v20210805.json')
