...
reference_data_set = [
    "default"
]  # ['default']  #, 'alternate1']  #, 'alternate', 'ref3']
ext = ".xml"  #'.nc'
ext = ".nc"
...
if exp == "historical" and MIP == "cmip5":
    filename_template = (
        MIP
        + ".historical.%(model).r1i1p1.mon.%(variable).198101-200512.AC."
        + modver
        + ".nc"
    )
if exp == "amip" and MIP == "cmip5":
    filename_template = (
        MIP + ".amip.%(model).r1i1p1.mon.%(variable).198101-200512.AC." + modver + ".nc"
    )
if exp == "historical" and MIP == "cmip6":
    filename_template = (
        MIP
        + ".historical.%(model).r1i1p1f1.mon.%(variable).198101-200512.AC."
        + modver
        + ".nc"
    )
if exp == "amip" and MIP == "cmip6":
    filename_template = (
        MIP
        + ".amip.%(model).r1i1p1f1.mon.%(variable).198101-200512.AC."
        + modver
        + ".nc"
    )
if exp == "picontrol":
    filename_template = (
        "%(variable)_%(model)_%(table)_picontrol_%(exp)_r1i1p1_01-12-clim.nc"
    )
...
custom_observations = os.path.abspath(
    "/p/user_pub/PCMDIobs/catalogue/obs4MIPs_PCMDI_clims_byVar_catalogue_v20210816.json"
)
...
filename_output_template = (
    ...
    + "."
    + realization
    + ".mo.%(variable)%(level).%(period).interpolated.%(regrid_method).%(region).AC."
    + case_id
    + "%(ext)"
)
