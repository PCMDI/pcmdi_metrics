...
from pcmdi_metrics.monsoon_old.lib import (
    AddParserArgument,
    YearCheck,
...
json_filename = "_".join(
    ["monsoon_sperber_stat_000", mip, exp, "%(model)", fq, realm, str(msyear) + "-" + str(meyear)]
)
            ...
            try:
                if model == "obs":
                    ...
                    if realization in ["all", "All", "ALL", "*"]:
                        ...
                        print(" run = model_path.split  =  : ", run)
                    else:
                        run = realization
                    # dict
                ...
                JSON = pcmdi_metrics.io.base.Base(
                    outdir(output_type="metrics_results"), json_filename.replace("%(model)", model)
                )
                JSON.write(
                    monsoon_stat_dic,
