...
pathin = '/work/cmip-dyn/' + MIP + '/CMIP/' + exp + '/atmos/day/pr/'

# METRICS LOCATION
outdir = '/p/user_pub/pmp/pmp_results/tree_v0.3/pmp_v1.1.2/metrics_results/pr/pr_dist_apendergrass/' + version + '/'
outdir = '/p/user_pub/pmp/pmp_results/tree_v0.3/pmp_v1.1.2/metrics_results/pr/pr_dist_apendergrass/' + MIP + '/' + exp + '/' +version + '/'
outdir = '/p/user_pub/pmp/pmp_results/pmp_v1.1.2/metrics_results/pr/pr_dist_apendergrass/' + MIP + '/' + exp + '/' +version + '/'

# DIAGNOSTICS LOCATION
diags_outdir = '/export_backup/gleckler1/processing/metrics_package/my_test/angie_pr_dist/diagnostics/'
diags_outdir = '/p/user_pub/pmp/pmp_results/tree_v0.3/pmp_v1.1.2/diagnostic_results/pr/pr_dist_apendergrass/' + MIP + '/' + exp + '/' + version + '/'
diags_outdir = '/p/user_pub/pmp/pmp_results/pmp_v1.1.2/diagnostic_results/pr/pr_dist_apendergrass/' + MIP + '/' + exp + '/' + version + '/'
...
# if l.find('MIROC4h') == -1 and l.find('MRI-AGCM3-2S') == -1: lst.append(l)
  if l.find('CNRM-CM5.r1i1p1') != -1: lst.append(l)
  print(l)
#lst = ['CMIP5.CMIP.historical.CNRM-CERFACS.CNRM-CM5.r1i1p1.day.pr.atmos.glb-2d-gu.v20120530.0000000.0.xml']
lst = ['CMIP6.CMIP.historical.CNRM-CERFACS.CNRM-CM6-1.r1i1p1f2.day.pr.atmos.glb-2d-gr.v20180917.0000000.0.xml']
print('Complete list of files is: ', lst)
print('DOMAIN IS ', domain)
#print "lst0 is ", lst[0]

###outdir = './json/'
#json_filename = 'apenderass_metrics_cmip5-' + exp + '_' + domain + '_' + `sy` +'.'+ `sm` + '.' + `sd` + '-' + `ey` + '.' + `em` + '.' + `ed` + '_' + regrid_tool + '_' + regrid_method + '_' + regrid_dim + '.json'
#if regrid != 'y': json_filename = 'apenderass_metrics_cmip5-' + exp + '_' + domain + '_' + `sy` +'.'+ `sm` + '.' + `sd` + '-' + `ey` + '.' + `em` + '.' + `ed` + '.json'
#
...
#diags_outdir = './diagnostics/'
#diags_outdir = '/export_backup/gleckler1/processing/metrics_package/my_test/angie_pr_dist/diagnostics/'
#diags_outdir = '/export/gleckler1/processing/metrics_package/my_test/angie_pr_dist/diagnostics/'
...
#  foc = '/p/user_pub/pmp/pmp_obs/obs/atm/mo/pr/GPCP/ac/pr_GPCP_000001-000012_ac.nc' 
   foc = '/p/user_pub/PCMDIobs/PCMDIobs2.0/atmos/mon/pr/GPCP-2-3/gn/v20190301/ac/pr_mon_GPCP-2-3_BE_gn_197901-201804_AC.nc'
   fo = cdms2.open(foc)
   do = fo('pr')
   fo.close()
 if regrid_dim == '1x1':
...
#   dic_tmp = {} 
#   if mod not in dic_tmp.keys(): dic_tmp[mod] = {}
#   dic_tmp[mod][rip]= {}
